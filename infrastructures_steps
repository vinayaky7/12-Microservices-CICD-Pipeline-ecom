...
```bash
eksctl create cluster --name=EKS-Cluster-Microservices \
                      --region=us-east-1 \
...
eksctl utils associate-iam-oidc-provider \
    --region us-east-1 \
    --cluster EKS-Cluster-Microservices \
    --approve

eksctl create nodegroup --cluster=EKS-Cluster-Microservices \
                       --region=us-east-1 \
                       --name=node2 \
                       --node-type=t3.medium \
                       --nodes=3 \
                       ...
                       --node-volume-size=20 \
                       --ssh-access \
                       --ssh-public-key=linkin-general-key \
                       --managed \
                       --asg-access \
                       --external-dns-access \
                       --full-ecr-access \
